...
impl Project {
    ...
    pub fn active_port(&self) -> Result<Option<u16>, Error> {
        match self {
            Self::Started(project_started) => {
                Ok(project_started.active_port().copied())
            }
            _ => Ok(None) // not ready
        }
    }
...
impl Service {
    pub async fn from_container<'c, C: Context<'c>>(container: ContainerInspectResponse, ctx: &C) -> Result<Self, Error> {
        ...
        let port = ctx.client(target).active_port(&name).await;
